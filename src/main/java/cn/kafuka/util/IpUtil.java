...
public class IpUtil
{
    ...
    {
        if (ObjUtil.isEmpty(addr) || addr.length < 2)
        {
            return true;
        }
    ...
    public static boolean checkIp(String ipAddr) {
        ...
        if(!ObjUtil.isEmpty(ipAddr)){
            result =  Pattern.matches("^([1-9]|[1-9]\\d|1\\d{2}|2[0-4]\\d|25[0-5])(\\.(\\d|[1-9]\\d|1\\d{2}|2[0-4]\\d|25[0-5])){3}$", ipAddr);
        }
        return result;
    }
